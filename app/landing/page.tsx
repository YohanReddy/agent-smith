...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#050505] text-[#e5e5e5] overflow-hidden relative">
      {/* Background grid */}
      <div className="absolute inset-0 bg-[linear-gradient(rgba(34,197,94,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(34,197,94,0.03)_1px,transparent_1px)] bg-[size:50px_50px] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_50%,black_40%,transparent_100%)]" />
      
      {/* Ambient glow */}
      <div className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-emerald-500/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-emerald-500/3 rounded-full blur-[100px] pointer-events-none" />
      ...
      <header className="relative z-10 flex items-center justify-between px-8 py-6 animate-[fadeIn_0.6s_ease-out_forwards] opacity-0">
        ...
        <nav className="flex items-center gap-6">
          <Link href="/" className="font-mono text-xs text-[#71717a] hover:text-[#e5e5e5] transition-colors">
            enter app
          </Link>
        </nav>
      </header>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-200px)] px-8">
        <div className="max-w-4xl text-center">
          ...
          <div className="animate-[fadeInUp_0.8s_ease-out_0.1s_forwards] opacity-0 translate-y-8">
            <h1 className="font-mono text-5xl md:text-7xl font-medium tracking-tight mb-6">
              <span className="text-[#e5e5e5]">build agents,</span>
              <br />
          ...
          <div className="animate-[fadeInUp_0.8s_ease-out_0.2s_forwards] opacity-0 translate-y-8">
            <p className="font-mono text-sm text-[#71717a] max-w-xl mx-auto mb-12 leading-relaxed">
        ...
        <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl animate-[fadeInUp_0.8s_ease-out_0.5s_forwards] opacity-0 translate-y-8">
          {[
            ...
            <div
              key={feature.title}
              className="p-4 border border-[#27272a] rounded bg-[#0a0a0a]/50 backdrop-blur-sm hover:border-emerald-500/30 transition-all duration-500 animate-[fadeIn_0.6s_ease-out_forwards]"
              style={{ animationDelay: `${0.6 + i * 0.1}s`, opacity: 0 }}
            >
              <div className="font-mono text-xs text-emerald-500 mb-2">0{i + 1}</div>
              <h3 className="font-mono text-sm text-[#e5e5e5] mb-1">{feature.title}</h3>
              <p className="font-mono text-[10px] text-[#52525b]">{feature.desc}</p>
            </div>
          ))}
        </div>
      </main>
